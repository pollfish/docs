...
# <span style="color:red">!This is a Beta version of the upcoming Pollfish iOS SDK v6!</span>


Since version 5 a lot has changed regarding the usage of Pollfish iOS SDK

Please read the migration guide [below](#migration-guide)
...
* Granted IDFA tracking permission (iOS 14+)

> **Note:** Pollfish cannot function without the usage of IDFA. Insructions on how to request permission can be found [here](#5.-request-idfa-tracking-permission)

</br>

## Quick Guide

1. [Obtain a Developer Account](#1.-obtain-a-developer-account)
...
3. [Download and import Pollfish.xcframework to your project](#3.-download-and-import-pollfish.xcframework-to-your-project)
4. [Add AdSupport.framework, AppTackingTransparency.framework, SystemConfiguration.framework and CoreTelephony.framework to your project](#4.-add-the-following-frameworks-(if-you-don’t-already-have-them)-in-your-project)
5. [Request IDFA tracking permission](#5.-Request-IDFA-Tracking-permission)
6. [Embed pollfish in your code and call init](#6.-Embedding-Pollfish-into-your-code)
7. [Set to **Release mode** and release in AppStore](#7.-Distributing-your-app-to-AppStore)
8. [Update your privacy policy](#8.-Update-your-App's-Privacy-Policy-in-the-app-and-on-AppStore-Connect)
9. [Request your account to get verified from Pollfish Dashboard](#9.-Request-your-account-to-get-verified)
...
> **Note:** This step is required only for iOS 14+ targeting. For more detailed usage instructions please visit this [repository](https://github.com/pollfish/ios-sdk-pollfish)
...
> **Note:** In order to receive Pollfish lifecycle events see [`PollfishDelegate` section](#11-listen-for-pollfish-lifecycle-events-(optional))

<span style="text-decoration: underline">Objective-C:</span>
```objc
- (void) applicationDidBecomeActive:(UIApplication *)application 
{
    PollfishParams *params = [[PollfishParams alloc] init];

    [params rewardMode:BOOL];
    [params releaseMode:BOOL];
    [params offerwallMode:BOOL];
    
    [Pollfish initWith:NSString params:params delegate:nil];
}
```
...
func applicationDidBecomeActive(application: UIApplication) {
    ...
    Pollfish.initWith(String, params: params)
}
```
...
struct ContentView: View {
    ...
    func initPollfish() {
        ...
        Pollfish.initWith("API_KEY", params: params)
    }
}
```

> **Note:** For instructions on how to define a ```SwiftUI``` ```PollfishView``` click [here](#6.2.4-`.pollfishViewContainer(UIView)`)
...
> **Note:** All the params are optional, except the **`releaseMode`** setting that turns your integration in release mode prior uploading to AppStore 
...
6.2.7 | **`.surveyFormat(SurveyFormat)`** <br/> Explicitly requests a specific survey format
6.2.8 | **`.offerwallMode(Bool)`** <br/> Sets Pollfish to offerwall mode
6.2.9 | **`.userProperties(UserProperties)`** <br/> Provides user attributes upfront during initialization
6.2.10| **`.rewardInfo(RewardInfo)`** <br/> An object holding information regarding the survey completion reward
6.2.11| **`.clickId(String)`** <br/> A pass throught param that will be passed back through server-to-server callback
...
For **SwiftUI** based you have to define a struct comforming to  ```UIViewRepresentable``` protocol and expose a UIView field to Pollfish initialisation. The following example demonstrates how to create a custom ```View``` and handle it's position on the view hierarchy.
...
```
For instructions on how to initialize with a custom ```View``` click [here](#6.2-Initialize-Pollfish)
...
#### **6.2.7 `.surveyFormat(SurveyFormat)`**
...
Below you can see an example on how you can explicitly request to test Pollfish Playful Survey Format in debug mode:
...
[params surveyFormat:SurveyFormatBasic];
```
...
> **Note:** It's preferable not to use this setting and let Pollfish  decide which surveys are more relevant to your users.

<br/>

#### **6.2.8 `.offerwallMode(Bool)`**
...
#### **6.2.9 `.userProperties(UserProperties)`**
...
#### **6.2.11 `clickId(String)`**

A pass through param that will be passed back through server-to-server callback as specified [here](https://www.pollfish.com/docs/s2s)
...
<span style="text-decoration: underline">Swift:</span>
```swift
et pollfishParams = PollfishParams()
    .clickId("CLICK_ID")
```
...
An optional parameter used to secure the `rewardName` and `rewardConversion` parameters as passed in the `RewardInfo` object (5.2.10)
...
After your app is published on AppStore you should request your account to get verified from your Pollfish Developer Dashboard.
...
**`rewardName`**         | A virtual reward name as specified on Developer Dashboard 
**`rewardValue`**        | A virtual reward value as caluclated via a given echange rate on Developer Dashboard 
...
You can be notified when a survey is not available for a user via the `PollfishDelegate`.  
...
```objc
[Pollfish initWith:@"API_KEY" params:params delegate:nil];
```
