...
- Google Play Services 17 and higher

> **Note:** Pollfish cannot function without the usage of Google advertising Id.
...
2. Register a new App on on Pollfish Developer Dashboard and copy the given API key
...
Download Pollfish Android SDK or reference it through maven repository.
...
Skipping this step will result on runtime exception on Pollfish initialization.
...
After linking your project to Google Play Services you can easily initialize Pollfish. In order to initialize, you will need to build an instance of Params using `Params.Builder`. `Params.Builder` has only one mandatory parameter which is the API key of your app (step 2 above). 
...
Once you have created `Params` instance, then you can call `Pollfish.initWith()` in `onResume()` function of your Activity (just after `super.onResume()`) and you are ready to go.
...
### 5.3.1 Pollfish Params available options (Optional)
...
> **Note:** All the params are optional, except the **`releaseMode`** setting that turns your integration in release mode prior publishing to the Google Play
...
This setting sets the psotion where you wish to place Pollfish indicator ![alt text](https://storage.googleapis.com/pollfish_production/multimedia/pollfish_indicator_small.png) <br/> Pollfish indicator is shown only if Pollfish is used in a non rewarded mode.
...
Sets a unique id to identify a user and be passed through server-to-server callbacks. You can read more on how to retrieve this param through the callbacks [here](https://www.pollfish.com/docs/s2s)
...
Here is an example of how a user can pass a view of an `Activity` through `Params.Builder` instance during initialization.
...
Initializes Pollfish in reward mode. This means that Pollfish Indicator will not be shown and Pollfish survey panel will be automatically hidden until the publisher explicitly calls Pollfish `show` function (The publisher should wait for the pollfish survey received callback). This behaviour enables the option for the publisher, to show a custom prompt to incentivize the user to participate to the survey.
...
You can view an example on how to achieve this behaviour [here](https://github.com/pollfish/android-sdk-pollfish)
...
**`rewardConversion`** | Overrides the reward conversion as specified in the Publisher's Dashboard. Conversion is expecting a number matching this function ( ```1 USD = X Points``` ) where ```X``` is a ```Double``` number.
...
An optional parameter used to secure the `rewardName` and `rewardConversion` parameters as passed through the `RewardInfo` object (5.3.1.9)
...
## 9. Listen for Pollfish lifecycle events

In order to get notified for Pollfish lifecycle events you will have to pass the appropriate listener in `Params.Builder` (section 5).
...
Implement `PollfishSurveyReceivedListener` Interface and override `onPollfishSurveyReceived` function
...
> **Note:** When offerwall mode is enabled, it returns a `null` object. It just notifies that surveys are available within the offerwall.
...
Implement `PollfishSurveyCompletedListener` Interface and override `onPollfishSurveyCompleted` function
...
Implement `PollfishUserNotEligibleListener` Interface and override `onUserNotEligible` function
...
Implement `PollfishSurveyNotAvailableListener` Interface and override `onPollfishSurveyNotAvailable` function
...
Implement `PollfishOpenedListener` Interface and override `onPollfishOpened` function
...
Implement `PollfishClosedListener` Interface and override `onPollfishClosed` function
...
You can be notified when a user rejected a survey

Implement `PollfishUserRejectedSurveyListener` Interface and override `onUserRejectedSurvey` function
