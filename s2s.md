...
5. Sort the values from (4) alphabeticaly using the names of the template parameters. The names of the template parameters are: `cpa`, `device_id`, `request_uuid`, `status`, `timestamp`, `term_reason` and `tx_id` and not the names of any [URL parameters](https://en.wikipedia.org/wiki/Query_string) your URL may contain.
