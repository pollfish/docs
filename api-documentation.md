...
|71 | **content_type** | String | If it is set to "json", it returns offerwall response with HTTP 200 in json format. If no surveys are found, it returns 204 with an empty body. This param, requires Offerwall parameter to be true. If is set to "html" or ommited, it defaults to Offerwall behavior. |  "json" / "html" | No | No
...
##### a. JSON offerwall request for a user with unknown demographics:
...
##### JSON offerwall request for a user, with all the demographic info already known
